...
export class RequestForPodPopupComponent implements OnInit {
  ...
  populateFieldsData() {
    ...
    this.formDefaultData.phone = phoneNumber;
    ...
    this.formDefaultData.invoice = this.data['InvoiceReference'];
    this.formDefaultData.documentNumber = this.data['docNumber'];
    this.formDefaultData.documentType = 'Invoice';
  }
    ...
    this.requestForPodForm = this.fb.group(
      {
        ...
        phone: new UntypedFormControl('', [Validators.minLength(6), Validators.maxLength(16)]),
  ...
  submit($event) {
    ...
    if (this.requestForPodForm.controls.phone.errors) {
